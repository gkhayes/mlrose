...
def generate_tsp(seed, number_of_cities, area_width=250, area_height=250):
    np.random.seed(seed)
    ...
    while len(duplicates) > 0:
        for d in duplicates:
            ...
            for i in range(len(d)):
                ...
                pass
        duplicates = list_duplicates_(coords)
    distances = get_tsp_distances(coords, False)
    return coords, distances


def get_tsp_distances(coords, truncate=True):
