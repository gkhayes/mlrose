...
class DiscreteOpt: 
    ...
    def eval_node_probs(self):
        ...
        # Get probs
        probs = np.zeros([self.length, self.max_int, self.max_int])
        ...
        if len(self.keep_sample) == 0:
            ...
            probs[0, :] = np.histogram(self.keep_sample[:, 0], \
                 np.arange(self.max_int + 1), density = True)[0]
        
        for i in range(1, self.length):
            for j in range(self.max_int):
    ...
    def find_neighbors(self):
        ...
        if self.max_int == 2:
            ...
            for i in range(self.length):
                vals = list(np.arange(self.max_int))
    ...
    def get_mate_probs(self):
        ...
        Returns:
        self.mate_probs: array. Numpy array containing mate probabilities of the 
        current population
        """
    ...
    def random(self):
        ...
        """
        state = np.random.randint(0, self.max_int, self.length)
    ...
    def random_neighbor(self):
        ...
        if self.max_int == 2:
            ...
            neighbor[i] = vals[np.random.randint(0, self.max_int-1)]
    ...
    def reproduce(self, parent_1, parent_2, mutation_prob = 0.1):
        ...
        mutation_prob: float. Probability of a mutation at each state element during reproduction.
        ...
        if self.max_int == 2:
            ...
            for i in mutate:
                vals = list(np.arange(self.max_int))
                vals.remove(child[i])
                child[i] = vals[np.random.randint(0, self.max_int-1)]
    ...
    def sample_pop(self, sample_size): 
        ...
        new_sample[:, 0] = np.random.choice(self.max_int, sample_size,\
        ...
        for i in sample_order:
            ...
            for j in range(self.max_int):
                ...
                new_sample[inds, i] = np.random.choice(self.max_int,\
                         len(inds), p = self.node_probs[i, j])
